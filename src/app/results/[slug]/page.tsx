      ...
      const fetchResultsData = async () => {
        ...
        try {
          ...
          if (!quizDocSnap.exists()) {
            toast({ title: "Quiz Not Found", description: "This quiz does not exist or was deleted.", variant: "destructive", duration: 7000 });
          ...
          toast({ title: "Error Loading Results", description: e.message || "Could not load data from the cloud.", variant: "destructive", duration: 7000 });
        }
        setIsLoading(false);
      };
  ...
  if (isLoading || isAuthorizedCreator === null) { 
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <p className="text-xl text-muted-foreground">Crunching the numbers...</p>
      </div>
    );
  }
  ...
  if (!isAuthorizedCreator) { 
     return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center p-6">
        ...
        <h2 className="text-3xl font-semibold mb-3">Leaderboard Access Restricted</h2>
        <p className="text-lg text-muted-foreground mb-8 max-w-md">
          Only the quiz creator, <span className="font-semibold text-foreground">{quizBaseData?.creatorName || 'them'}</span>, can view the full leaderboard. If you took the quiz, you saw your score already!
        </p>
        <Link href="/">
  ...
  if (!quizBaseData || !quizBaseData.questionsUsed) { 
    return (
       <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-center p-6">
        ...
        <h2 className="text-3xl font-semibold mb-3">Quiz Data Incomplete</h2>
        <p className="text-lg text-muted-foreground mb-8 max-w-md">
          The essential information for this quiz (like questions or creator name) could not be loaded from the cloud. Results cannot be displayed.
        </p>
         <Link href="/">
          <Button size="lg" className="text-lg py-7 px-8">
            <Home className="mr-2 h-5 w-5" />
      ...
      <div className="flex flex-col items-center space-y-8 py-8 sm:py-12">
        <Card className="w-full max-w-3xl shadow-xl border-2 border-primary/20">
          <CardHeader className="text-center pt-8">
            ...
            <CardTitle className="text-3xl sm:text-4xl font-bold">Quiz Results Dashboard</CardTitle>
            <CardDescription className="text-lg sm:text-xl pt-2 text-muted-foreground">
              See who knows <span className="font-semibold text-foreground">{quizBaseData.creatorName}</span> best!
              <span className="block text-sm mt-1">(Shows attempts made by anyone who used your share link)</span>
            </CardDescription>
          </CardHeader>
          <CardContent className="px-4 sm:px-6">
            {sortedAttempts.length > 0 && quizBaseData.questionsUsed.length > 0 ? (
                ...
                <Table className="min-w-full">
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      <TableHead className="w-[80px] text-center text-lg font-semibold">Rank</TableHead>
                      <TableHead className="text-lg font-semibold">Friend's Name</TableHead>
                      <TableHead className="text-right text-lg font-semibold">Score</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
              ...
              <div className="text-center py-12 space-y-4">
                ...
                <p className="text-xl font-semibold text-muted-foreground">No attempts recorded yet.</p>
                <p className="text-md">Share your quiz link to get some results on the board!</p>
              </div>
            )}
          </CardContent>
          <CardFooter className="flex-col sm:flex-row justify-between items-center gap-4 px-6 sm:px-8 py-6 border-t mt-4">
            <Link href={`/share/${slug}`} className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="w-full text-lg py-3 h-auto border-primary text-primary hover:bg-primary/10">
                  <Share2 className="mr-2 h-5 w-5" /> Share Quiz Again
              </Button>
            </Link>
            <Link href="/" className="w-full sm:w-auto">
              <Button size="lg" className="w-full text-lg py-3 h-auto">
                <Home className="mr-2 h-5 w-5" /> Create a New Quiz
              </Button>
            </Link>
          </CardFooter>
        </Card>
