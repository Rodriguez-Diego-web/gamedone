...
export default function QuizBuilderPage() {
    ...
    if (partiallyCompletedQuiz) {
        try {
            ...
            if (savedAnswers && savedQuestions && typeof savedNextIndex === 'number') {
                ...
                toast({
                    title: "Welcome Back!",
                    description: "Resuming your quiz creation.",
                    duration: 3000,
                });
            } else {
  ...
  const handleAnswerSubmit = async (answer: string) => {
    ...
    if (updatedCreatorAnswers.length === TOTAL_QUESTIONS_TO_PICK) {
      ...
      try {
        ...
        toast({
          title: "Quiz Saved to Cloud!",
          description: "Your quiz structure is now securely saved online.",
          duration: 3000,
          variant: "default"
        });
      } catch (error) {
        ...
        toast({
          ...
          description: "Could not save quiz to the cloud. It's saved locally for now.",
          variant: "destructive",
          duration: 5000,
        });
      }
      ...
      toast({
        title: "Quiz Created!",
        description: "Your quiz is ready to be shared.",
        duration: 5000,
      });
  ...
  if (isLoading || !creatorName) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] space-y-4">
        ...
        <p className="text-lg text-muted-foreground">Loading quiz setup...</p>
      </div>
    );
  }
     ...
     if(availableQuestions.length < DUMMY_QUESTIONS.length){ 
        ...
        return (
          <div className="flex flex-col items-center justify-center min-h-[calc(100vh-400px)] space-y-4 text-center p-4">
            ...
            <p className="text-xl font-semibold text-destructive">Oops! We've run out of unique questions for now.</p>
            <p className="text-md text-muted-foreground">You've answered {creatorAnswers.length} questions. We recommend creating a quiz with this many, or refreshing to try for more.</p>
            <div className="flex gap-4 mt-4">
              <Button onClick={() => router.refresh()} variant="outline">Try Refreshing</Button>
              {creatorAnswers.length > 0 && (
                 <Button onClick={async () => {
                    ...
                    toast({ 
                        title: "Quiz Created!",
                        description: "Your quiz (with fewer questions) is ready to be shared.",
                        duration: 5000,
                     });
                    router.push(`/share/${quizSlug}`);
                 }}>
                    Finalize Quiz with {creatorAnswers.length} Questions
                 </Button>
              )}
            </div>
          </div>
  ...
  if (creatorAnswers.length >= TOTAL_QUESTIONS_TO_PICK) {
    return (
       <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] space-y-4">
        <Loader2 className="h-12 w-12 animate-spin text-primary" />
        <p className="text-lg text-muted-foreground">Finalizing your awesome quiz...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center space-y-8 py-8">
      <div className="w-full max-w-2xl space-y-3">
        <p className="text-center text-lg font-medium text-primary">
          Hi {creatorName}, let's define your answers!
        </p>
        ...
        <p className="text-center text-md text-muted-foreground font-medium">
          Questions Answered: {creatorAnswers.length} of {TOTAL_QUESTIONS_TO_PICK}
        </p>
      </div>
      ...
      {availableQuestions[currentQuestionIndex] ? (
        ...
        />
         ...
         <div className="flex flex-col items-center justify-center min-h-[calc(100vh-400px)] space-y-4">
            <Loader2 className="h-12 w-12 animate-spin text-primary" />
            <p className="text-lg text-muted-foreground">Summoning the next question...</p>
         </div>
      )}
    </div>
  );
